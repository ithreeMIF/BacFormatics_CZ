...
% Last Modified by GUIDE v2.5 16-Dec-2015 06:09:29
...
regexprep(filename, 'BacFormatics_SETUP_','')                  
full_filename= strcat(pathname,'BacFormatics_SETUP_',filename) ;                  
full_filename=char(full_filename);                  
...
find_ind=findstr(pathname , filesep,filesep) ;                  
...
set(handles.listbox1,'String',filename_str2);                  
set(handles.listbox1,'Max',ii) ;                  
set(handles.listbox1,'Value',1) ;                  
set(handles.listbox1,'Min',0);                  
 close(h)     
...
set(h,'color','w');                  
for ii=1:size(filename_str,2)                  
timebar_BACWrapper(h,ii/size(filename_str,2) )                  
...
data_file(3).cdata(3,2)=cellstr(get(handles.edit_ch02,'string'));                  
data_file(3).cdata(4,2)=cellstr(get(handles.edit_ch03,'string'));                  
data_file(7).cdata=cell(6,3) ; %flags for tracking and labeling status (with label is easy only has to use isempty(centy1) command                  
...
data_file(7).cdata(1,3)=cellstr('N');                  
data_file(7).cdata(2,3)=cellstr('N');                  
data_file(7).cdata(3,3)=cellstr('N');                  
data_file(7).cdata(4,3)=cellstr('N');                  
data_file(7).cdata(5,3)=cellstr('N');                  
data_file(7).cdata(6,3)=cellstr('N');   
...
str=pathname( strfind(pathname,filesep,'pos'):end);                  
str_index=find(ismember(str,filesep)) ;                  
str(str_index)=[]  ;                  
more_str=get(handles.define_section_name,'String');                  
filename=strcat(pathname,'BacFormatics_EXP_New_',str,more_str ,'.dat')  ;                  
...
pathname_temp= strcat(pathname,'ch00_Filtered',filesep);                  
set(handles.ch00_Filtered_folder,'String',pathname_temp);                  
pathname_temp= strcat(pathname,'ch01_Filtered',filesep);                  
set(handles.ch01_Filtered_folder,'String',pathname_temp);                  
pathname_temp= strcat(pathname,'ch02_Filtered',filesep);                
set(handles.ch02_Filtered_folder,'String',pathname_temp);                  
pathname_temp= strcat(pathname,'ch03_Filtered',filesep);             
set(handles.ch03_Filtered_folder,'String',pathname_temp);                  
pathname_temp= strcat(pathname,'ch00_Segmented',filesep);                 
set(handles.ch00_Segmented_folder,'String',pathname_temp);                  
pathname_temp= strcat(pathname,'ch01_Segmented',filesep);                   
set(handles.ch01_Segmented_folder,'String',pathname_temp);                  
pathname_temp= strcat(pathname,'ch02_Segmented',filesep);                 
set(handles.ch02_Segmented_folder,'String',pathname_temp);                  
pathname_temp= strcat(pathname,'ch03_Segmented',filesep);                 
...
%    return;                  
% end                  
pathname =char(strcat(listbox1(nn),filesep));                  
...
a=char(pathname);                  
try                  
str=a(findstr(a,filesep,'Pos')+4:end)                  
...
str=a(findstr(a,filesep,'Pos')+1:end);                  
more_str=get(handles.define_section_name,'String');                  
if isempty(str)~=1                  
filename=char(strcat(pathname,filesep,'BacFormatics_EXP_New_',more_str,'_',str,'.dat'))                  
else                  
filename=char(strcat(pathname,filesep,'BacFormatics_EXP_New_',more_str,'.dat'))                  
end                  
%This it the principle: the temp experiment should not be overwriiten                  
%completed experiments file by mistake.                  
...
guidata(hObject, handles);                  
str=strcat(pathname,filesep,'*.tif') ;                  
dir_filename=dir(str)  ;                  
...
pathname=get(handles.pathname,'string');                  
try                  
full_filename = char(strcat( pathname,char( box_Raw(n)))) ;  temp=imread_cross(full_filename );                  
catch                  
full_filename = char(strcat( pathname,filesep,char( box_Raw(n)))) ;  temp=imread_cross(full_filename  );                  
end                  
try                  
axes(handles.axes1);                  
cla(handles.axes1);   
 
channel=str2num(full_filename(end-5:end-4)) +1  ;
switch channel
    case 1
        channel2 =get(handles.ch_00,'value') 
    case 2
         channel2 =get(handles.ch_01,'value') 
    case 3
         channel2 =get(handles.ch_02,'value') 
    case 4
         channel2 =get(handles.ch_03,'value') 
end
...
full_filename = char(strcat(pathname,filesep,char( box_Raw(n)))) ;  temp=imread(full_filename,'tif',1);                  
end                  
...
pathname=get(handles.pathname,'string');                  
str=handles.pathname( strfind(pathname,filesep,'pos'):end);                  
str_index=find(ismember(str,filesep)) ;                  
str(str_index)=[]  ;                  
more_str=get(handles.define_section_name,'String');                  
filename=strcat(pathname,'BacFormatics_EXP_New_',str,more_str ,'.dat')  ;                  
...
full_filename = char(strcat(pathname,filesep,char( box_list(n)))) ;  temp=imread(full_filename,'tif',1);                  
end                  
axes(handles.axes1);                  
cla(handles.axes1);                  
