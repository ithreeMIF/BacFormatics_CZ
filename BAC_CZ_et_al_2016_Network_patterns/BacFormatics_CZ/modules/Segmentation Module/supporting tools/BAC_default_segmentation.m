...
function varargout = BAC_default_segmentation(varargin)                  
% BAC_default_segmentation M-file for BAC_default_segmentation.fig                  
%      BAC_default_segmentation, by itself, creates a new BAC_default_segmentation or raises the existing                  
%      singleton*.                  
%                  
%      H = BAC_default_segmentation returns the handle to a new BAC_default_segmentation or the handle to                  
%      the existing singleton*.                  
%                  
%      BAC_default_segmentation('CALLBACK',hObject,eventData,handles,...) calls the local                  
%      function named CALLBACK in BAC_default_segmentation.M with the given input arguments.                  
%                  
%      BAC_default_segmentation('Property','Value',...) creates a new BAC_default_segmentation or raises the                  
%      existing singleton*.  Starting from the left, property value pairs are                  
%      applied to the GUI before BAC_default_segmentation_OpeningFcn gets called.  An                  
%      unrecognized property name or invalid value makes property application                  
%      stop.  All inputs are passed to BAC_default_segmentation_OpeningFcn via varargin.                  
%                  
%      *See GUI Options on GUIDE's Tools menu.  Choose "GUI allows only one                  
%      instance to run (singleton)".                  
%                  
% See also: GUIDE, GUIDATA, GUIHANDLES                  
% Edit the above text to modify the response to help BAC_default_segmentation                  
% Last Modified by GUIDE v2.5 20-Apr-2015 15:50:53
% Begin initialization code - DO NOT EDIT                  
gui_Singleton = 1;                  
gui_State = struct('gui_Name',       mfilename, ...                  
'gui_Singleton',  gui_Singleton, ...                  
'gui_OpeningFcn', @BAC_default_segmentation_OpeningFcn, ...                  
'gui_OutputFcn',  @BAC_default_segmentation_OutputFcn, ...                  
'gui_LayoutFcn',  [] , ...                  
'gui_Callback',   []);                  
...
% --- Executes just before BAC_default_segmentation is made visible.                  
function BAC_default_segmentation_OpeningFcn(hObject, ~, handles, varargin)                  
...
eval(strcat('set(handles.Section_',num2str(iii),',''String'',  C) '));                  
 ...
 try
set(handles.Raw_listbox,'string',varargin{1})
end


% --- Outputs from this function are returned to the command line.                  
function varargout = BAC_default_segmentation_OutputFcn(~, ~, handles)                  
...
[filename, pathname, filterindex] = uigetfile({  '*.dat','dat-files (*.dat)';}, 'Please Choose setting file' )% handles.directory_name); %pick files to combine                  
...
set(experiment_H,'Position',Position)     
 ...
 filename_str=char(filename_str(1));
for ii=1:1 
 filename_str(1)=[] 
 importdata_file=importdata(char( filename_str))
