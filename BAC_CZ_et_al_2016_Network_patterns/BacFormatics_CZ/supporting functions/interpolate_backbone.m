...
function [cell_centerline,dist_cell_centerline]=interpolate_backbone(ROI_bw,pole1_x,pole1_y,pole2_x,pole2_y,step,Npoints,span )
...
if max(D(:))==1
    ...
    if max(L(:))==4
        disp('aberrant cell/segment shape.') 
    end
    [X,Y] =find(L==3);
    for  ii=1:max(size(X))
        X1=X(ii) ;
